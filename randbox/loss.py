...
class SetCriterionDynamicK(nn.Module):
    """ This class computes the loss for RandBox.
    ...
    def __init__(self, cfg, num_classes, matcher, weight_dict, eos_coef, losses, use_focal):
        ...
        self.start_count = 0
        self.start_iter = cfg.MODEL.RandBox.CHANGE_START
        ...
        if self.use_focal:
            self.focal_loss_alpha = cfg.MODEL.RandBox.ALPHA
            self.focal_loss_gamma = cfg.MODEL.RandBox.GAMMA
        else:
    ...
    def loss_nc_labels(self, outputs, targets, indices, num_boxes, log=False):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        assert 'pred_logits' in outputs
    ...
    def forward(self, outputs, targets):
        ...
        # Compute the average number of unknown_target boxes accross all nodes, for normalization purposes
        unknown_num_boxes = sum(len(t["labels"]) for t in unknown_targets)
        unknown_num_boxes = torch.as_tensor([unknown_num_boxes], dtype=torch.float, device=next(iter(outputs.values())).device)
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(unknown_num_boxes)
        unknown_num_boxes = torch.clamp(unknown_num_boxes / get_world_size(), min=1).item()
        ...
        for loss in self.losses:
            if loss == 'nc_labels':
                if self.start_count > self.start_iter:
                    losses.update(self.get_loss(loss, outputs, unknown_targets, ow_indices, unknown_num_boxes))
            else:
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == 'nc_labels':
                        if self.start_count > self.start_iter:
                            l_dict = self.get_loss(loss, aux_outputs, unknown_targets, ow_indices, unknown_num_boxes, **kwargs)
...
#                 ==========================================================
                ...
                unknown_indices_batchi_b = torch.tensor([0]*len(unknown_label),dtype=torch.long,device=unknown_indices_batchi_a.device)
